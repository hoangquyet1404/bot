...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    try {
        const res = (await axios.get(`https://api.fgmods.xyz/api/search/xvideosearch?text=${encodeURIComponent(search)}&apikey=vQTmzp7i`)).data;
...
module.exports.handleReply = async function ({ event, api, Currencies, Users, handleReply }) {
    ...
    try {
        const res = await axios.get(`https://8d2c-101-32-128-19.ngrok-free.app/api/downloader/xvideosdl?url=${encodeURIComponent(chosenVideo.url)}`);
        const response = res.data.result;
        const description = response.views;
        const uploadDate = response.likes;
        const title = response.title;

        
        // Kiểm tra dung lượng video
        const headRes = await axios.head(url_dl);
        ...
        const videoResponse = await axios.get(url_dl, { responseType: "arraybuffer" });
