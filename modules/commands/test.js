...
module.exports.getHentaiz = async function(type, first, second) {
    ...
    switch(type) {
        case '1': {
            var list = (await axios.get(`https://api.sumiproject.net/hentaiz/list`)).data;
            return list
        }
        case '2': {
            var read = (await axios.get(`https://api.sumiproject.net/hentaiz/read?id=${first}&chapter=${second}`)).data;
            return read
        }
        case '3': {
