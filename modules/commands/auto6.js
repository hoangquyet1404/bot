...
/*AUTODOWN MEDIAFILE VIIDEO */
      ...
      const res = (await axios.get(`https://api.fgmods.xyz/api/downloader/twitter?url=${str}&apikey=vQTmzp7i`)); 
       let attachment = [];
                      if (data.data != null) {
      attachment = await streamURL(res.data.result.HD,"mp4")
      } else {
      attachment = await streamURL(res.data.result.thumb, 'jpg');
                      }
      send({body: `${head("TWITTER")}\n→ Tiêu đề: ${res.data.text}`,attachment: download});
      }
